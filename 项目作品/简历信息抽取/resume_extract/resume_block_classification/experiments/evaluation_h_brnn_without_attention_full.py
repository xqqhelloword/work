...
from resume_block_classification.experiments.evaluation_h_brnn import AttentionLayer
...
class GRUModel(tf.keras.Model):
    ...
    def call(self, x, training=True):
        ...
        for i, item in enumerate(tf.unstack(x1, axis=1)):
            ...
            # h = tf.stack([out_brnn, out_fnn], axis=1)  # (sample_size, 2, 64)
            ...
            # out_brnn = self.dense_brnn(out_brnn)
            # out_fnn = self.dense_fnn(out_fnn)
            # outputs_fnn.append(out_fnn)
            # outputs_brnn.append(out_brnn)
            # out = self.attention(h)
            # print('out:',out)
            out = self.dense(out)
            # print('after attention,out.shape:', out.shape)
            # out = self.batch_normalization1(out)
            ...
            outputs.append(out)
        # return tf.stack(outputs, axis=1), tf.stack(outputs_brnn, axis=1), tf.stack(outputs_fnn, axis=1)  # 最终结果shape:(batchsz, time_step, 11)
    ...
    def my_loss(actual, predict):
        ...
        for i in range(length):
            total_loss += GRUModel.cct(actual_items[i], predict_items[i])
        return total_loss
...
if __name__ == '__main__':
    def evaluation_mbrnn():
        train_x, train_y, test_x, test_y = load_data_for_rnn_new_add_noise(data_set=3, train_num=600, test_num=400, noise_percent=10, noise_type='swap')
        brnn = GRUModel(time_step=12, feature_size=100, rnn_utils=64, rnn_layers_num=2)
        begin = 0
    ...
    def evaluation_mbrnn_load_model():
        train_x, train_y, test_x, test_y = load_data_for_rnn_new_add_noise(data_set=3, train_num=100, test_num=400, noise_percent=10, noise_type='shuffle')
        brnn = GRUModel(time_step=TIME_STEP, feature_size=WORD2VEC_FEATURE_NUM, rnn_utils=RNN_UTILS, rnn_layers_num=RNN_LAYERS_NUM)
    ...
    def test_model():
        ...
        brnn_save = GRUModel(time_step=12, feature_size=100, rnn_utils=64, rnn_layers_num=1)
